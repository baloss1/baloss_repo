...
tf.compat.v1.enable_eager_execution()  # Solve the issue "TypeError: Input must be a SparseTensor." here
...
J = 8 #Change to modify the number of cluster
...
class LinearBanditVariableCollection(tf.Module):
  ...
  def __init__(self,
    ...
    self.cur_cluster = 0
    self.cov_matrix_list = [[] for j in range(J)]
    self.data_vector_list = [[] for j in range(J)]
    self.eig_matrix_list = [[] for j in range(J)]
    self.eig_vals_list = [[] for j in range(J)]
    self.num_samples_list = [[] for j in range(J)]
    self.clusters = []   # A list contains of a set of clusters

    # Initialization of sets of clusters
    for j in range(J):
    ...
    for delay in range(500):
        for loss in range(100):
            for goodput in range(1200):
                c = random.randint(0,J-1)
                tmp_tuple = (delay, loss,goodput)
                #tmp_list = tf.constant([delay, loss, goodput], dtype=tf.int64)
                self.clusters[c].add(tmp_tuple)
    print("Finish clustering here.")
    #for j in range(J):
    #    self.clusters[j] = tf.convert_to_tensor(self.clusters[j], dtype=tf.int64)
                
    for k in range(num_models):
      for j in range(J):
          self.cov_matrix_list[j].append(
              tf.compat.v2.Variable(
                  tf.zeros([context_dim, context_dim], dtype=dtype),
                  name='a_' + str(k)))
          self.data_vector_list[j].append(
              tf.compat.v2.Variable(
                  tf.zeros(context_dim, dtype=dtype), name='b_{}'.format(k)))
          self.num_samples_list[j].append(
              tf.compat.v2.Variable(
                  tf.zeros([], dtype=dtype), name='num_samples_{}'.format(k)))
          if use_eigendecomp:
              self.eig_matrix_list[j].append(
                  tf.compat.v2.Variable(
                      tf.eye(context_dim, dtype=dtype),
                      name='eig_matrix{}'.format(k)))
              self.eig_vals_list[j].append(
                  tf.compat.v2.Variable(
...
def update_a_and_b_with_forgetting(
  ...
  #print(" This is the new update_a_and_b_with_forgetting function. ################################33")
...
class LinearBanditAgent(tf_agent.TFAgent):
  ...
  def __init__(self,
    ...
    self._clusters = variable_collection.clusters # Initialization of clusters.
    ...
    self._cur_cluster = variable_collection.cur_cluster
    ...
    policy = lin_policy.LinearBanditPolicy(
        ...
        cov_matrix=self._cov_matrix_list[self._cur_cluster],
        data_vector=self._data_vector_list[self._cur_cluster],
        num_samples=self._num_samples_list[self._cur_cluster],
  ...
  @property
  def cur_cluster(self):
      return self._cur_cluster
      ...
      thetas.append(
          tf.squeeze(
              linalg.conjugate_gradient_solve(
                  self._cov_matrix_list[self._cur_cluster][k] + self._tikhonov_weight *
                  tf.eye(self._overall_context_dim, dtype=self._dtype),
                  tf.expand_dims(self._data_vector_list[self._cur_cluster][k], axis=-1)),
              axis=-1))
  ...
  def _distributed_train_step(self, experience, weights=None):
    ...
    for k in range(self._num_models):
      ...
      def _merge_fn(strategy, per_replica_cov_matrix_update,
        ...
        updates_and_vars = [
            (per_replica_cov_matrix_update, self._cov_matrix_list[self._cur_cluster][k]),
            (per_replica_data_vector_update, self._data_vector_list[self._cur_cluster][k])
        ]
        ...
        self._cov_matrix_list[self._cur_cluster][k].assign_add(reduced_updates[0])
        self._data_vector_list[self._cur_cluster][k].assign_add(reduced_updates[1])

        # Compute the eigendecomposition, if needed.
        if self._use_eigendecomp:
          eig_vals[self._cur_cluster], eig_matrix[self._cur_cluster] = tf.linalg.eigh(self._cov_matrix_list[self._cur_cluster][k])
          self._eig_vals_list[self._cur_cluster][k].assign(eig_vals[self._cur_cluster])
          self._eig_matrix_list[self._cur_cluster][k].assign(eig_matrix[self._cur_cluster])
  ...
  def _train(self, experience, weights=None):
    ...
    for k in range(self._num_models):
      # Observe the current context u_t and its cluster C_t  (Line 8 in Algorithm)
      def get_global_observation_from_observation(observation):
          # This is the global observation set up.
          # We first turn it into an array and then set
          # three parameters as [delay, loss, goodput]. Notice this should be
          # in the same order as we used three parameters in clusters.
          # The array is turned into tuple at last.

          global_observation_array = observation.numpy()
          global_observation = global_observation_array[0][0:3]
          #global_observation = global_observation.tolist()
          global_observation[0] = math.floor(global_observation[0])
          global_observation[1] *= 100
          global_observation[1] = math.floor(global_observation[1])
          global_observation[2] /= 1000
          global_observation[2] = math.floor(global_observation[2])
          global_observation = tuple(global_observation)
          """
          global_observation = tf.slice(observation, [0,0], [1,3])
          global_observation_float = tf.cast(global_observation, dtype=tf.float64)
          multiple_matrix = tf.constant([1,0,0,0,100,0,0,0,0.001], shape=[3,3], dtype=tf.float64)
          global_observation_answer = tf.matmul(global_observation_float, multiple_matrix)
          global_observation = tf.cast(global_observation_answer, dtype=tf.int64)
          print("Here, global_observation: ", global_observation)
          return global_observation
          """
          return global_observation
      def observe_cluster(observation):
          global_observation = get_global_observation_from_observation(observation)
          i = 0
          print("This is the tuple global observation: ", global_observation)
          for cluster in self._clusters:
              if global_observation in cluster:
                  print("The observation is : ", global_observation, "The number of cluster is: ", i)
                  return i 
              i += 1
          return -1 
          ...
          for j in range(J):
            print("here la.")
            for i in range(len(self._clusters[j])):
                if tf.equal(global_observation, self._clusters[j][i]).numpy().all():
                    print("The observation is : ", global_observation, "The number of cluster is: ", j)
                    return j
          return -1
          """
      ...
      observations_for_arm = tf.matmul(diag_mask, observation)
      rewards_for_arm = tf.matmul(diag_mask, tf.reshape(reward, [-1, 1]))
      self._cur_cluster = observe_cluster(observation)
      num_samples_for_arm_current = tf.reduce_sum(diag_mask)
      tf.compat.v1.assign_add(self._num_samples_list[self._cur_cluster][k],
                              num_samples_for_arm_current)
      num_samples_for_arm_total = self._num_samples_list[self._cur_cluster][k].read_value()
      ...
      a_new, b_new, eig_vals, eig_matrix = tf.cond(
          ...
          lambda: update(self._cov_matrix_list[self._cur_cluster][k], self._data_vector_list[self._cur_cluster][k]),
          lambda: (self._cov_matrix_list[self._cur_cluster][k], self._data_vector_list[self._cur_cluster][k],
                   self._eig_vals_list[self._cur_cluster][k], self._eig_matrix_list[self._cur_cluster][k]))

      tf.compat.v1.assign(self._cov_matrix_list[self._cur_cluster][k], a_new)
      tf.compat.v1.assign(self._data_vector_list[self._cur_cluster][k], b_new)
      tf.compat.v1.assign(self._eig_vals_list[self._cur_cluster][k], eig_vals)
      tf.compat.v1.assign(self._eig_matrix_list[self._cur_cluster][k], eig_matrix)
      # Update the matrix of per arm feature weight <thetas> here:
      thetas = self.theta
      # Add line 17~18 in algorithm here.
      # Calculate k' and argMin_k' here. (This is the wrong way to update theta.)
      """
      thetas = []
      for j in range(J):
          thetas.append(
              tf.squeeze(
                  linalg.conjugate_gradient_solve(
                      self._cov_matrix_list[j][0] + self._tikhonov_weight *
                      tf.eye(self._overall_context_dim, dtype=self._dtype),
                      tf.expand_dims(self._data_vector_list[j][0], axis=-1)),
                  axis=-1))
      """
      # Calculate k' here. (This is the wrong way to calculate k')
      # j_p = tf.argmin(thetas)

      # Calculate k' here. (This is the correct way to calculate k')
      j_p = -1
      min_norm = 0
      for j in range(J):
          cov_matrix_list_tmp = self._cov_matrix_list
          data_vector_list_tmp = self._data_vector_list
          eig_vals_list_tmp = self._eig_vals_list
          eig_matrix_list_tmp = self._eig_matrix_list
          a_j_p, b_j_p, eig_vals_j_p, eig_matrix_j_p = tf.cond(
              tf.squeeze(num_samples_for_arm_total) > 0,
              lambda: update(cov_matrix_list_tmp[j][k], data_vector_list_tmp[j][k]),
              lambda: (cov_matrix_list_tmp[j][k], data_vector_list_tmp[j][k],
                       eig_vals_list_tmp[j][k], eig_matrix_list_tmp[j][k]))

          tf.compat.v1.assign(cov_matrix_list_tmp[j][k], a_j_p)
          tf.compat.v1.assign(data_vector_list_tmp[j][k], b_j_p)
          tf.compat.v1.assign(eig_vals_list_tmp[j][k], eig_vals_j_p)
          tf.compat.v1.assign(eig_matrix_list_tmp[j][k], eig_matrix_j_p)

          # Calculate theta_ts (theta_k' in the paper)
          theta_ts = []
          for k in range(self._num_models):
              theta_ts.append(
                  tf.squeeze(
                      linalg.conjugate_gradient_solve(
                          cov_matrix_list_tmp[j][k] + self._tikhonov_weight *
                          tf.eye(self._overall_context_dim, dtype=self._dtype),
                          tf.expand_dims(data_vector_list_tmp[j][k], axis=-1)),
                      axis=-1))
          theta_ts = tf.stack(theta_ts, axis=0)

          if j_p == -1:
              # Calculate min_norm between thetas and theta_ts
              min_norm = tf.norm(thetas - theta_ts, axis=-1)
              j_p = j
          else:
              # Calculate min_norm_tmp between thetas and theta_ts
              min_norm_tmp = tf.norm(thetas - theta_ts, axis=-1)
              if (min_norm_tmp < min_norm):
                  min_norm = min_norm_tmp
                  j_p = j

      # This is the wrong way to calculate theta_ts
      """        
      theta_ts = []
      for j in range(J):
          theta_ts.append(
          tf.squeeze(
              linalg.conjugate_gradient_solve(
                  a_new + self._tikhonov_weight *
                  tf.eye(self._overall_context_dim, dtype=self._dtype),
                  tf.expand_dims(b_new, axis=-1)),
              axis=-1))
      """
      # Calculate the norm between the difference of thetas and theta_t
      # TODO: POSSIBLE BUG HERE
      # This is the wrong way to calculate norm
      """
      theta_diff_norms = []
      for j in range(J):
          thetas[j] = thetas[j] - theta_ts[j]
          theta_diff_norms.append(tf.norm(thetas[j], ord='euclidean', axis=None, keepdims=None))
      print("This is thetas: ",thetas)
      # Calculate k' here.
      j_p = tf.argmin(theta_diff_norms)
      """

      # Finish calculation in line 17 here
      if (self._cur_cluster != j_p):
          self._clusters[j_p].add(get_global_observation_from_observation(observation))
          self._clusters[self._cur_cluster].remove(get_global_observation_from_observation(observation))
